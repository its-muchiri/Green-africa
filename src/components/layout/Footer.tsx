...
export function Footer() {
  ...
  return (
    <footer className="bg-jungle-900 relative overflow-hidden noise-bg">
      <div className="relative mx-auto max-w-[1400px] px-4 sm:px-6 lg:px-8 py-16 sm:py-24">
        <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-5">
          <div className="lg:col-span-2">
            ...
            <address className="not-italic text-ink-muted font-mono text-xs leading-relaxed max-w-xs">
              <div className="font-display font-medium text-jungle-100 text-sm mb-2">Visit Us</div>
          ...
          <div>
            <h4 className="font-display font-semibold text-lg mb-4 tracking-tight-display text-jungle-100">Company</h4>
            <nav aria-label="Company links">
              <ul className="space-y-3 font-mono text-xs uppercase tracking-wider">
                {footerLinks.company.map((link) => (
                  <li key={link.href}>
                    <Link href={link.href} className="text-ink-muted hover:text-clay-400 transition-colors">
                      {link.label}
                    </Link>
                  </li>
                ))}
          ...
          <div>
            <h4 className="font-display font-semibold text-lg mb-4 tracking-tight-display text-jungle-100">Services</h4>
          ...
          <div>
            <h4 className="font-display font-semibold text-lg mb-4 tracking-tight-display text-jungle-100">Connect</h4>
              ...
              <ul className="space-y-3 font-mono text-xs uppercase tracking-wider">
                {footerLinks.connect.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="text-ink-muted hover:text-clay-400 transition-colors flex items-center gap-2"
            ...
            <div className="mt-8 pt-8 hairline-divider-subtle">
              <div className="font-display font-medium text-sm mb-2 text-jungle-100">Hours</div>
              <ul className="font-mono text-xs text-ink-muted space-y-1">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-6 flex-wrap">
            <a href={siteConfig.email} className="font-mono text-xs text-ink-muted hover:text-clay-400 transition-colors">
              {siteConfig.email}
            </a>
            <a href={`tel:${siteConfig.phone.replace(/\s/g, "")}`} className="font-mono text-xs text-ink-muted hover:text-clay-400 transition-colors">
              {siteConfig.phone}
            </a>
            <div className="flex items-center gap-4" aria-label="Social links">
              {socialLinks.map((social) => (
                <a
                  ...
                  className="text-ink-muted hover:text-clay-400 transition-colors"
                ...
                >
                  ...
                  {social.icon === "instagram" && (
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><rect x="2" y="2" width="20" height="20" rx="5" ry="5" strokeWidth="2" /><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z" strokeWidth="2" /><line x1="17.5" y1="6.5" x2="17.51" y2="6.5" strokeWidth="2" /></svg>
                  )}
                  ...
                  {social.icon === "youtube" && (
                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M23 4.5a3 3 0 0 0-3-3h-14a3 3 0 0 0-3 3v15a3 3 0 0 0 3 3h14a3 3 0 0 0 3-3v-15z" /><path d="M9 12l6 4.5-6 4.5V12z" /></svg>
                  )}
                </a>
              ))}
            </div>
